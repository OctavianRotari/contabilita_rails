module Features
  def company(name, category_id = 1)
    Company.create(name:name,adress:'Ravenna',number:'345', category_id: category_id, user_id: 1)
  end

  def vehicle(plate="ER341GF")
    Vehicle.create(plate:plate,type_of_vehicle:'trattore', user_id: 1)
  end

  def category(category="officina")
    Category.create(category: category, user_id: 1)
  end
